...
function removeLastOccurrences(str, value) {
  const indexSub = str.lastIndexOf(value);
  if (str === '') {
    return '';
  }
  if (indexSub > -1) {
    return str.slice(0, indexSub) + str.slice(indexSub + value.length);
  }
  return str;
}
...
function countVowels(str) {
  const vowels = ['a', 'e', 'i', 'o', 'y', 'u', 'A', 'E', 'I', 'O', 'Y', 'U'];
  let n = 0;
  const strArr = str.split('');
  for (let i = 0; i < vowels.length; i += 1) {
    for (let k = 0; k < strArr.length; k += 1) {
      if (vowels[i] === strArr[k]) {
        n += 1;
      }
    }
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    let substr = '';
    for (let k = 0; k < strArr[i].length; k += 1) {
      if (
        strArr[i][k] === ',' ||
        strArr[i][k] === '!' ||
        strArr[i][k] === '?' ||
        strArr[i][k] === '.' ||
        Number.isInteger(+strArr[i][k]) ||
        strArr[i][k] === '@' ||
        strArr[i][k] === '#' ||
        strArr[i][k] === '$'
      ) {
        ...
        if (strArr[i].split('')[k] === strArr[i][k].toUpperCase()) {
          substr += strArr[i][k].toLowerCase();
        }
        if (strArr[i].split('')[k] === strArr[i][k].toLowerCase()) {
          substr += strArr[i][k].toUpperCase();
        }
      }
    }
    invertArr.push(substr);
  }
...
function extractNameFromTemplate(value) {
  ...
  return `${strArr[1]} ${strArr[2].replace('!', '')}`;
}
